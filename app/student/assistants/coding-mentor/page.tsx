...
export default function CodingMentor() {
  const router = useRouter();
  const [gradeLevel, setGradeLevel] = useState('');
  const [selectedTopic, setSelectedTopic] = useState('');

  const topics = gradeLevel ? ['Python', 'JavaScript', 'Web Development', 'Java', 'C++', 'App Development'] : [];
      ...
      <main className="flex-1 ml-64 p-8 overflow-y-auto">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
              <h1 className="text-3xl font-bold text-indigo-600">Coding Mentor</h1>
              <p className="text-gray-600">Learn programming, web development, and more</p>
            </div>
          </div>
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
            ...
            <div className="mb-6">
              ...
              <select value={gradeLevel} onChange={(e) => setGradeLevel(e.target.value)} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
            ...
            {!gradeLevel ? (
              ...
              <div className="grid grid-cols-2 gap-3 mb-6">
                {topics.map((topic) => (
                  <button key={topic} onClick={() => setSelectedTopic(topic)} className={`p-4 border-2 rounded-lg text-left transition-all ${selectedTopic === topic ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200 hover:border-gray-300'}`}>
            ...
            {selectedTopic && (
              <button onClick={() => router.push('/student/ai-tutor')} className="w-full mt-6 bg-indigo-600 text-white py-3 rounded-lg font-semibold hover:bg-indigo-700 transition-colors">
